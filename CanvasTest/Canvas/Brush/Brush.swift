...
open class Brush {
    ...
    // randering color, same color to the color prop erty with alpha reseted to alpha * opacity
...
//        print("render linestrip on target")
    ...
    internal func render(buffer: MTLBuffer, commandEncoder: MTLRenderCommandEncoder, vertexCount: Int = 1, on renderTarget: RenderTarget? = nil) {
        ...
        commandEncoder?.drawPrimitives(type: .point, vertexStart: 0, vertexCount: vertexCount)
        commandEncoder?.endEncoding()
